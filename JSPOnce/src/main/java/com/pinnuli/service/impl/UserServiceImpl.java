...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public String doRegister(String username, String password, String email, String province, HttpSession session) {
        String msg;

        //校验验证码
        String randomString = (String) session.getAttribute(ValidateCodeUtil.RANDOM_CODE_KEY);
        // 这里还需要调整！！
        if (userDaoImpl.checkProvince(username)) {
            msg = "注册失败，验证码错误";
            return msg;
        }
        ...
        UserBean user = new UserBean(username, email, password,province,0);
        //将用户保存到数据库，保存成功则开个线程的方式给用户发送一封邮件
        if (userDaoImpl.save(user) > 0) {
            new Thread(new MailUtil(email, province)).start();
            msg = "注册成功，请登录邮箱激活账号";
            return msg;
        }
        msg = "注册失败，请检查相关信息";
    ...
    @Override
    public String doLogin(String username, String password, String province, HttpSession session) {
        String msg;
        UserBean user = new UserBean(username, password,province);
     ...
     //       msg = "登录失败，验证码错误";
      ...
      //  }
        ...
        if (userDaoImpl.checkUsernameAndPasswordAndProvince(user)) {
            //登录成功把用户放到session中
            session.setAttribute("currentUser", user.getUsername());
            String stringA=user.getUsername()+user.getPassword()+user.getProvince();
            msg = "登录成功"+stringA;
            return msg;
        }
        if (userDaoImpl.checkUsername(user.getUsername())) {
            String stringA=user.getUsername()+user.getPassword()+user.getProvince();
            msg = "登录失败，密码错误"+stringA;
            return msg;
        }
        msg = "登录失败，用户不存在";
        return msg;
