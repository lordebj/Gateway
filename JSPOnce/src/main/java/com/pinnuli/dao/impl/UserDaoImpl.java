...
public class UserDaoImpl implements UserDao {
    ...
    public int save(UserBean user) {
        ...
        try {
            ...
            String sql = "insert into user(username,email,password,state,province) values(?,?,?,?,?)";
    ...
    public boolean checkProvince(String province) {
        ...
        try {
            Connection conn = DBUtil.getConnection();
            String sql = "select * from user where province = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
    ...
    @Override
    public boolean checkUsernameAndPasswordAndProvince(UserBean user) {
        ResultSet rs;
        boolean checkResult = false;
        try {
            Connection conn = DBUtil.getConnection();
            String sql = "select * from user" +
                    " where username = ? and password = ? and province = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, user.getUsername());
